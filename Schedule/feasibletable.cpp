...
FeasibleTable::FeasibleTable(vector<Course> course, vector<Venue> rooms, int maxPeriod,  int periods_in_day, vector<Curricula> curCodes)
{
	this->check = true;
	this->course = course;
...
vector<Course> FeasibleTable::getCourse() const {
	return course;
}

vector<string> FeasibleTable::getCurCodes() const {
	return curCodes;
}

int FeasibleTable::getMaxPeriod() const {
	return maxPeriod;
}

int FeasibleTable::getPeriodsInDay() const {
	return periods_in_day;
}
...
vector<RoomTime> FeasibleTable::getVenueTime() const {
	return venueTime;
}
	...
	while (courseIterator != this->course.end())
	{
		...
		*/
		...
		while (roomTimeIterator != this->venueTime.end())
		{
			...
			std::vector<string>::iterator findRoom = std::find(roomConstr.begin(), roomConstr.end(), roomCode);

			//check if period constraint exists
			std::vector<int>::iterator findPeriod = std::find(unAvailConstr.begin(), unAvailConstr.end(), period);

			// can put in room (exists in constraint list) and can put in period (not exist in unavail constraint list)
			if (findRoom != roomConstr.end() && findPeriod == unAvailConstr.end())
			{
				this->adjacentMatrix[i][j] = this->t_max;
				//cout<<i<<"\t"<<j<<"\t"<<this->adjacentMatrix[i][j]<<" "<<c->getCode()<<" "<<rt.toString()<<endl;
 ...
 */
void FeasibleTable::antGraphWalk(vector<CourseRoomTime>* timetable, int cycle, int* num_hcv, int* num_scv)
{
	...
	//std::srand(rSeed);
	SM s;
	std::random_shuffle(copyCourse.begin(), copyCourse.end(), myrandomF); //shuffle courses to select random to schedule
	...
	while (copyCourseIterator != copyCourse.end())
	{
		...
		for (int i =0; i < numWeeklyLectures; i++)
		{
			...
			RoomTime rt = antCourseWalk(c, tabuRoomTime, tabuDays, curriculaPeriod, lecturerPeriod, cycle, tempTimetable);
			CourseRoomTime courseroomtime(c, rt);
			courseroomtime.setUpdate(this->check);



			std::vector<CourseRoomTime>::iterator findCRT = std::find(timetable->begin(), timetable->end(), courseroomtime);
			...
			std::vector<RoomTime>::iterator findRT = std::find(tabuRoomTime.begin(), tabuRoomTime.end(), rt);
			...
			while (itcCodes != cCodes.end())
			{
				...
				std::vector<string>::iterator findCode_str = std::find(curriculaPeriod[rt.getPeriod()].begin(), curriculaPeriod[rt.getPeriod()].end(), code_str);
			...
			std::vector<string>::iterator findLecturer = std::find(lecturerPeriod[rt.getPeriod()].begin(), lecturerPeriod[rt.getPeriod()].end(), c.getLecturer());
	...
	//cout<<endl<<"********************"<<endl;
	ImproveTable impTable(getVenueTime(),rooms, getCurCodes(), getCourse(), *timetable, getMaxPeriod(), getPeriodsInDay());
 ...
 */
RoomTime FeasibleTable::antCourseWalk(Course c, vector<RoomTime> tabuVenueTime, list<int> tabuDays, vector<string> curriculaPeriod[], vector<string> lecturerPeriod[], int cycle, vector<CourseRoomTime> timetable)
{
	this->check = true;
	vector<RoomTime> adjacentVertices = getVenueTimeVertices(c);  //get possible roomtimes for course
	//cout<<adjacentVertices.size()<<" adjacentVertices"<<endl;
	vector<RoomTime> L1Filter = filterVenueListTabu(tabuVenueTime, adjacentVertices);  //remove roomperiods already assigned
	//cout<<L1Filter.size()<<" L1Filter"<<endl;
	vector<RoomTime> L2Filter = this->filterCurriculumPeriodTabu(c, curriculaPeriod, L1Filter); //remove RoomPeriods that will cause curriculum clash
	//cout<<L2Filter.size()<<" L2Filter"<<endl;
	vector<RoomTime> L3Filter = this->filterLecturerPeriodTabu(c, lecturerPeriod, L2Filter); //remove RoomPeriods that will cause lecturer clash
	//cout<<L3Filter.size()<<" L3Filter"<<endl;
	vector<RoomTime> L4Filter = this->filterDayListTabu(L3Filter, tabuDays);  //try to meet minimum days constraint
	...
	for (std::size_t i =0; i < L4Filter.size(); i++)
	{
		...
		std::vector<RoomTime>::iterator findRT = std::find(tabuVenueTime.begin(), tabuVenueTime.end(), rtTemp);
		...
		std::vector<string>::iterator findLecturer = std::find(lecturerPeriod[rtTemp.getPeriod()].begin(), lecturerPeriod[rtTemp.getPeriod()].end(), c.getLecturer());
		...
		std::list<int>::iterator findDay = std::find(tabuDays.begin(), tabuDays.end(), rtTemp.getDay(this->periods_in_day));
		...
		while (cCurrIterator != cCurr.end() && !l)
		{
			...
			std::vector<string>::iterator findCurriculum = std::find(curriculaPeriod[rtTemp.getPeriod()].begin(), curriculaPeriod[rtTemp.getPeriod()].end(), curCod);
		...
		int conViolate = SM::getConsecutiveViolations(temp, rtTemp.getPeriod(), this->getPeriodsInDay());
		conViolate-=this->curCodes.size();
*/
		double trail = this->adjacentMatrix[this->getAdjacentCourseIndex(c)][this->getAdjacentRoomTimeIndex(rtTemp)];
		//double heuristic = ((2.3 - studentClash + venueClash + lecturerClash + dayConstraint) * conViolate) - conViolate;
 ...
 * That is filtered, meaning it does not contain any venue time element in the tabuList
 * Tabulist is the list of all venue time elements already assigned to a course
 */
vector<RoomTime> FeasibleTable::filterVenueListTabu(vector<RoomTime> tabuList, vector<RoomTime> vt)   //VenueTime Tabu Filter
{
	...
	while (rtIterator  !=  vt.end())
	{
		...
		std::vector<RoomTime>::iterator findRT = std::find(tabuList.begin(), tabuList.end(), rt);
...
vector<RoomTime> FeasibleTable::getVenueTimeVertices(const Course &c)
{
	...
	int amount =0;
	std::vector<Course>::iterator findIter = std::find(this->course.begin(), this->course.end(), c);
	...
	//cout<<adjacent.size()<<" The number of possible venue times "<< amount<<endl;
	return adjacent;
}
 ...
 * @return
 * Returns a new arrayList of venue times
 * That is filtered, meaning it does not contain any venue time element that that will result in a curriculum clash
 */
vector<RoomTime> FeasibleTable::filterCurriculumPeriodTabu(Course c, vector<string> curriculaPeriod[], vector<RoomTime> fVenueTabu)
{
	...
	while (rtIterator != fVenueTabu.end())
	{
		...
		while (ccIterator != curriculumCodes.end() && l)
		{
			...
			std::vector<string>::iterator findCode = std::find(curriculaPeriod[rt.getPeriod()].begin(), curriculaPeriod[rt.getPeriod()].end(), code);
 ...
 * @return
 * Returns a new arrayList of venue times
 * That is filtered, meaning it does not contain any venue time element that that will result in a lecturer clash
 */
vector<RoomTime> FeasibleTable::filterLecturerPeriodTabu(Course c, vector<string> lecturerPeriod[], vector<RoomTime> fCurPerTabu)
{
	...
	{
		RoomTime rt = *rtIterator;
		std::vector<string>::iterator findLecturer = std::find(lecturerPeriod[rt.getPeriod()].begin(), lecturerPeriod[rt.getPeriod()].end(), lecturer);

		if (findLecturer == lecturerPeriod[rt.getPeriod()].end()) //if not assigning same lecturer to same period
 ...
 */
vector<RoomTime> FeasibleTable::filterDayListTabu(vector<RoomTime> roomTimes, list<int> dayTabu)
{
	...
	{
		RoomTime rt = *rtIterator;
		std::list<int>::iterator findDay = std::find(dayTabu.begin(), dayTabu.end(), rt.getDay(this->periods_in_day));
...
{
	std::vector<RoomTime>::iterator findRT = std::find(this->venueTime.begin(), this->venueTime.end(), rt);
 ...
 */
RoomTime FeasibleTable::selectRoom(double probability[], vector<RoomTime> rt, int cycle)
{
	//cout<<"Here"<<endl;
	//cout<<rt.size()<<"\t";
	int length = rt.size();
 ...
 * RC - if two courses are assigned to the same room in the same period, two courses in the same venue
 * SC - Students are assigned multiple courses in the same period, two curriculums in the same period
 ...
 * SC - Size of venue assigned to a course is less than number of students taking the course
 * UPC - Course assigned a period when the lecturer is not available to teach a course
 ...
 * @return
 */
vector<Course> FeasibleTable::getPeriodCourse(int period, vector<CourseRoomTime> timetable)
{
	vector<Course> cees;
	...
	while (tIterator != timetable.end())
	{
		...
		if (c.getVenueTime().getPeriod() == period && c.getCourse().isEmpty() == false)
		{
 ...
 * Venue name
 */
string FeasibleTable::getVenueCourse(int period, Course c, vector<CourseRoomTime> timetable)
{
	string v = "";
	...
	while (crtIterator != timetable.end())
	{
		...
		if (cous == c && crt.getVenueTime().getPeriod() == period && cous.isEmpty() == false)
...
void FeasibleTable::evapourate()
{
	...
	for (std::size_t i =0; i < this->course.size(); i++)
	{
		for (std::size_t j=0; j < this->venueTime.size(); j++)
		{
			...
			if (this->adjacentMatrix[i][j] < this->t_min && this->adjacentMatrix[i][j] != 0)
				this->adjacentMatrix[i][j] = this->t_min;
		}
	}
}

void FeasibleTable::updateAntTrail(int timetableQuality, vector<CourseRoomTime> timetable)
{
	double reward = 1 / (1 + (double)timetableQuality - (double)this->globalBest);

	if (this->globalBest > timetableQuality)
		reward = 1;

/*	//Positive and negative rewards
	double reward = (1 / (1 + (double)timetableQuality - (double)this->globalBest )) - 1;
	reward = reward/10;

	if (this->globalBest > timetableQuality)
		//reward = 1;
		reward = 1 - (1 / (1 + (double)this->globalBest  - (double)timetableQuality));*/
	...
	while (crtIterator != timetable.end())
	{
		...
		adjacentMatrix[indexCourse][indexRoomTime] += reward; //Add reward based on quality of solution
...
int FeasibleTable::NumberHCV()
{
	return this->NumberHCV(this->feasibleTable);
}
...
void FeasibleTable::antColonyThread(int numAnts, int numCycles)
{
	...
	ImproveTable tempTable(getVenueTime(),rooms, getCurCodes(), getCourse(), globe, getMaxPeriod(), getPeriodsInDay());
	bool found = false;
	int number = 0;
	...
	{
		//int cycleb = 5000000;
		vector<CourseRoomTime>* antWalk = new vector<CourseRoomTime>[numAnts]; //create vector of results
		int num_hcv[numAnts]; //create vector of number of hard constraint violations
		int num_scv[numAnts]; //create vector of number of soft constraint violations
		std::vector<std::thread> th;
	    for(int j = 0; j < numAnts; j++) //create threads of number of ants to run in parallel
	    {
	    	th.push_back(std::thread(&FeasibleTable::antGraphWalk, this, &antWalk[j], ((i+1)*(j+1)), &num_hcv[j], &num_scv[j]));
	    }
	    ...
	    for (int k = 0; k < numAnts; k++)
	    {
	    	...
	    	cout<<"Cycle: " << i << " Ant: " << k << " Num HCV violations: " << num_hcv[k] <<"   Num SCV Violations: " << num_scv[k]<<" Ant Value: "<<antValue<<endl;
			//if (num_scv[k] == 0 && num_hcv[k] == 0)
	    	if (num_hcv[k] == 0)
			{
				globe =antWalk[k];
				this->feasibleTable=globe;
				cout<<"Found Global Best is: " << this->globalBest<< " in cycle "<< i <<  " Size: "<<globe.size() << " SCV: "<<num_scv[k]<< " HCV: "<<num_hcv[k]<<endl;
				delete[] antWalk;
				return;
			}
	    }
	    ...
	    //update trail
		cout<<this->globalBest<<" "<<antWalk[cycleBestIndex].size() << " Size of cycle best "<<" Quality of cycle best ["<<num_hcv[cycleBestIndex]<<" , "<<num_scv[cycleBestIndex]<<" ]"<<endl;
	...
	cout<<"Found Global Best is: " << this->globalBest<< " in "<< numCycles << " cycles "<<endl;
...
void FeasibleTable::antColony(int numAnts, int numCycles)
{
	...
	for (int i =0; i < numCycles; i++)
	{
		...
		for (int j =0; j < numAnts; j++)
		{
			...
			cout<<"Cycle: " << i << " Ant: " << j << " Num HCV violations: " << hcv_ant<<endl;
			if (scv_ant == 0 && hcv_ant == 0)
			{
				this->globalBest = hcv_ant;
				globe = cyclebest;
				cout<<"Found Global Best is: " << this->globalBest<< " in cycle "<< i <<endl;
				this->feasibleTable = globe;
				return;
			}
			if (hcv_ant == 0)
