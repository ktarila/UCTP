...
ImproveTable::ImproveTable(vector<RoomTime> venueTime, vector<Venue> rooms, vector<string> curCodes, vector<Course> course, vector<CourseRoomTime> timetable, int maxPeriod,  int periods_in_day)
{
	this->maxPeriod = maxPeriod;
	this->periods_in_day = periods_in_day;
	this->fullSchedule = timetable;
	this->course = course;
	...
	//add emptyCourses to each venue time to enable moves
	//cout<<"adding empty courses to timetable"<<endl;
	for (auto &vt:this->venueTime)
	{
		Course c;
		CourseRoomTime crt(c, vt);
		this->fullSchedule.push_back(crt);
	}
...
int ImproveTable::NumberSCV(const vector<CourseRoomTime>& timetable,  int* rsv, int* cwdv, int* clv, int *sv)
{
...
vector<CourseRoomTime> ImproveTable::getCRTsinDay(const int &day, const vector<CourseRoomTime> &timetable)
{
...
vector<string> ImproveTable::curriculumInTimetable(const vector<CourseRoomTime>& timetable)
{
...
vector<Course> ImproveTable::coursesInTimetable(const vector<CourseRoomTime>& timetable)
{
...
int ImproveTable::numberCurriculumLastTimeSlot(const vector<CourseRoomTime>& timetable)
{
...
vector<CourseRoomTime> ImproveTable::getCRTsinPeriod(const int& period, const vector<CourseRoomTime>& timetable)
{
...
vector<CourseRoomTime> ImproveTable::getKempeChain(const vector<CourseRoomTime>& timetable, const vector<CourseRoomTime>& swapList, const int& p1, const int& p2)
{
	vector<CourseRoomTime> crtinPeriodOne;
	vector<CourseRoomTime> crtinPeriodTwo;
	vector<CourseRoomTime> kempeChain;
	auto merged = getCRTsinGivenPeriods(p1, p2, timetable, &crtinPeriodOne, &crtinPeriodTwo);
	auto barpatiteMatrix = getBarpatiteMatrix(merged, crtinPeriodOne, crtinPeriodTwo);
    ...
    while (!toVisit.empty())
    {
    	...
    	for (size_t j =0; j < merged.size(); j++)
    	{
    		...
    		{
    			std::vector<CourseRoomTime>::iterator findCRT = std::find(kempeChain.begin(), kempeChain.end(), merged[j]);
...
vector<CourseRoomTime> ImproveTable::getCRTsinGivenPeriods(const int& p1, const int& p2, const vector<CourseRoomTime>& timetable, vector<CourseRoomTime> *periodOne, vector<CourseRoomTime> *periodTwo)
{
	vector<CourseRoomTime> crtinPeriodOne;
	vector<CourseRoomTime> crtinPeriodTwo;
...
vector<vector<int> > ImproveTable::getBarpatiteMatrix(vector<CourseRoomTime> merged, vector<CourseRoomTime> p1, vector<CourseRoomTime> p2)
{
...
vector<Venue> ImproveTable::getTakenRooms(int period, vector<CourseRoomTime> timetable)
{
	vector<Venue> taken; 
	for (auto &crt:timetable)
	{
...
vector<Venue> ImproveTable::getAvailableRooms(int period, vector<CourseRoomTime> timetable, vector<CourseRoomTime> kempeChain)
{
	...
	for (auto &r: takenAll)
	{
		available.erase(std::remove(available.begin(), available.end(), r), available.end());
	}
...
vector<CourseRoomTime> ImproveTable::singleKempeSwap(vector<Venue>* availableRooms, CourseRoomTime crt, int newPeriod, std::vector<CourseRoomTime> timetable)
{
...
std::vector<CourseRoomTime> ImproveTable::kempeChainSwap(vector<CourseRoomTime> kempeChain, vector<CourseRoomTime> timetable, int p1, int p2)
{
...
bool ImproveTable::hasCommon(Course c1, Course c2)
{
	...
	{
		std::vector<string>::iterator findCurriculum = std::find(c2Curriculum.begin(), c2Curriculum.end(), curr);
	 ...
	 ofstream myfile(name);
	  if (myfile.is_open())
	  {
		  ...
		  //cout<<"last day is: "<< lastDay << "  "<<this->maxPeriod;
		  	...
		  	{
		  		//cout<<" Day "<<i;
		  		vector<CourseRoomTime> daysubsetTable = this->getCRTsinDay(i, this->fullSchedule);  //course room time elements in day i
				...
				int endPeriod = startPeriod + this->periods_in_day -1;  //get last period in day i
		  		for (int j = startPeriod; j <= endPeriod; j++)
		  		{
		  			...
		  			for (auto &crt:periodsubsetTable)
		  			{
		  				...
		  				myfile<<courseCode<<"-"<<roomCode<<"-"<<crt.getVenueTime().getPeriod()<<"----";
		  			}
		  			myfile<<"\t";
		  		}
		  		myfile<<endl;
...
void ImproveTable::writeTimetableToFileSolutionFormat(string solutionName, vector<CourseRoomTime> timetable)
{
...
int ImproveTable::getIndex(CourseRoomTime crt, vector<CourseRoomTime> timetable)
{
	std::vector<CourseRoomTime>::iterator findCRT = std::find(timetable.begin(), timetable.end(), crt);
	int index = std::distance(timetable.begin(), findCRT);

	return index;
}

vector<CourseRoomTime> ImproveTable::removeCRTinPeriod(const int& period, vector<CourseRoomTime> timetable)
{
	...
	for (auto &c:timetable)
	{
		...
		{
			auto cc = c;
			std::vector<CourseRoomTime>::iterator findCRT = std::find(periodCRTs.begin(), periodCRTs.end(), c);
...
void ImproveTable::softWalk(const vector<CourseRoomTime>& timetable, const int& antIndex, vector<CourseRoomTime>* newSchedule, int* num_scv, vector<Path>* newPath, bool accept)
{
	...
	//std::srand(rSeed);
	std::random_shuffle(copySchedule.begin(), copySchedule.end(), myrandomI); //shuffle courses to select random to schedule
	//for (auto &crt:copySchedule)
	for (auto &crt:copySchedule)
	{
		Path tempPath(1, tempCRT, tempCRT);
		if (crt.getCourse().isEmpty() == false)
		{
			unsigned seed1 = std::chrono::system_clock::now().time_since_epoch().count() * antIndex;
			vector<CourseRoomTime> newSchedule;
			auto selectP = this->selectSwapCRT(schedule, crt, seed1, &newSchedule, accept);
			if (selectP.empty == false)
			{
				//auto swapCRT = schedule[selectP];
				//cout<<newSchedule.size()<<" size new Shed"<<endl;
				Path p(1, crt, selectP);
				//schedule = p.applyPath(schedule, &tempPath);
				schedule = newSchedule;
				//cout<<" swapped "<< crt.toString() <<" and "<< swapCRT.toString()<<endl;
				path.push_back(p);
			}
		}
	}
	//cout<<" The length of path is "<<path.size()<<" The length of schedule is "<<schedule.size()<<endl;
...
vector<CourseRoomTime> ImproveTable::getClashList(const vector<CourseRoomTime>& timetable, const CourseRoomTime& crt)
{
...
CourseRoomTime ImproveTable::selectSwapCRT(vector<CourseRoomTime> timetable, const CourseRoomTime& rowCRT, const int& antIndex, vector<CourseRoomTime>* newSched, bool accept)
{
	vector<double> heuristics;
	CourseRoomTime emptyCRT;
	//vector<CourseRoomTime> schedule = this->fullSchedule;
	vector<CourseRoomTime>::iterator findCRT = std::find(timetable.begin(), timetable.end(), rowCRT);
	//vector<CourseRoomTime> scheduleResults[this->fullSchedule.size()];
	vector<vector<CourseRoomTime> >  scheduleResults;
	int minSCV = this->NumberSCV(timetable);
	double allowance = 0;
	int maxSCV = 0;
	if (findCRT == timetable.end())
	{
		//auto x = rowCRT;
		//cout<<" already swapped: "<<x.toString()<<endl;;
		return emptyCRT;
	...
	findCRT = std::find(fullSchedule.begin(), fullSchedule.end(), rowCRT);
	if (findCRT == fullSchedule.end())
		return emptyCRT;
	int rowIndex = std::distance(fullSchedule.begin(), findCRT);
	vector<CourseRoomTime> functions;
	CourseRoomTime tempCRT;
	Path tempPath(1, tempCRT, tempCRT);

	bool l = accept;
	for (auto &colCRT:timetable)
	{
		//vector<CourseRoomTime>::iterator findColCRT = std::find(timetable.begin(), timetable.end(), this->fullSchedule[j]);
		...
		//auto rCRT = rowCRT;
		//cout<<rCRT.toString()<<" new: "<<newRowCRT.toString()<<endl;
		//cout<<colCRT.toString()<<" new: "<<newColCRT.toString()<<endl;
		int rowViolations =  0;
		int colViolations = 0;
		SM::periodViolation(newT, this->maxPeriod, newRowCRT.getVenueTime().getPeriod(), &rowViolations);
		if (newRowCRT.getVenueTime().getPeriod() != newColCRT.getVenueTime().getPeriod())
			SM::periodViolation(newT, this->maxPeriod, newColCRT.getVenueTime().getPeriod(), &colViolations);
		else if (newRowCRT.getVenueTime().getPeriod() == newColCRT.getVenueTime().getPeriod())
			colViolations = rowViolations;
		if (rowViolations == 0 && colViolations == 0)
		{
			functions.push_back(colCRT);
			double h = double(this->NumberSCV(newT));
			heuristics.push_back(h);
			scheduleResults.push_back(newT);
			if ( h < minSCV)
				minSCV = h;
			if (h > maxSCV)
				maxSCV = h;
		...
		if (colViolations > 0)
			cV = 1;
		int vSum = rV + cV;
		if (vSum == 1 && l == true) //repair single period violations
		//if (l == true)
		{
			//allowance = 1;

			CourseRoomTime violationCRT = newRowCRT;
			if (rV == 1 && cV == 0) //violation is in rowCRT
			{
				violationCRT = newRowCRT;

			}
			else if (rV == 0 && cV == 1) //violation ins in colCRT
			{
				violationCRT = newColCRT;
			}
			auto clashList = this->getClashList(newT, violationCRT);
			if (clashList.size() == 1) //if single CRT violation -- move crt to best position
			{
				auto copyNewTemp = newT;
				...
				//auto swapHCV = SM::NumberHCV(copyNewTemp, this->maxPeriod);
				if (newC.empty == true )
				{
					//cout<<" Safely swapped "<<endl;
					//int nSCurrent = this->NumberSCV(newTemp);
					//cout<<"HCV: "<<swapHCV<<endl;
					functions.push_back(colCRT);
					double h = double(this->NumberSCV(copyNewTemp));
					heuristics.push_back(h);
					scheduleResults.push_back(copyNewTemp);
				}
			}
	...
	if (functions.size() == 0)
		return emptyCRT;
	if (functions.size() == 1) //only one possible
	{
		*newSched = scheduleResults[0];
		return functions[0];

	}


	vector<double> probability(functions.size());


	//return the minimum test
/*	auto minCRT = functions[0];
	auto minSCV = heuristics[0];
	auto maxSCV = heuristics[0];
	for (std::size_t ind =0; ind < functions.size(); ind++)
	{
		if (heuristics[ind] < minSCV)
		{
			minSCV = heuristics[ind];
			minCRT = functions[ind];
		}
		if (heuristics[ind] > minSCV)
		{
			maxSCV = heuristics[ind];
		}
	}*/


	//get minimum CRTs then select one based on trail

	if (this->NumberSCV(timetable) == 1)
		minSCV += allowance;
	vector<CourseRoomTime> newFunctions;
	vector<vector<CourseRoomTime> > newScheduleResults;
	vector<double>  newHeuristics;
	for (std::size_t he_ind = 0;  he_ind < heuristics.size(); he_ind++)
	{
		double val =0;
		if (heuristics[he_ind] <= minSCV)
		{
			val = allowance - heuristics[he_ind];
			newFunctions.push_back(functions[he_ind]);
			newScheduleResults.push_back(scheduleResults[he_ind]);
			newHeuristics.push_back(val);
		}
		if (heuristics[he_ind] == 0)
			return functions[he_ind];
	}
	//return minCRT;

/*
	//Make heuristics to be between 0 and t_max
	double sumHeuristic = 0;
	for (std::size_t i =0; i < heuristics.size(); i++)
	{
		//cout<<heuristics[i]<<"("<<functions[i]<<"), ";
		sumHeuristic += heuristics[i];
		//if (max < heuristics[i])
			//max = heuristics[i];
	}
	//cout<<endl;
	...
	{
		//cout<<heuristics[i]<<"("<<functions[i]<<"), ";
		heuristics[i]/=sumHeuristic;
		heuristics[i]*=this->t_max;
		//cout<<heuristics[i]<<"  ";
...
/*
	double max = heuristics[0];
	double min = heuristics[0];
	for (std::size_t i =0; i < heuristics.size(); i++)
	{
		if (max < heuristics[i])
			max = heuristics[i];
...
*/
	...
	for (std::size_t i =0; i < newFunctions.size(); i++)
	{

		double trail = this->t_min;
		auto findCol = find(fullSchedule.begin(), fullSchedule.end(), newFunctions[i]);
		if (findCol != fullSchedule.end())
		{
			int colIndex = distance(fullSchedule.begin(), findCol);
			trail = this->adjacentMatrix[rowIndex][colIndex];
		}

		//normalize heuristics so lower scv has higher chance of select
		//double he = maxSCV - heuristics[i] + minSCV;
		//cout<<trail<<" "<<feasibleIndex<<" "<<periodIndex<<"  Heuristics: "<<he<<",     ";
		//cout<<he<<" "<<trail<<endl;


		double trailFactor = pow(trail, this->beta);
		double heuristicFactor = pow(newHeuristics[i], this->alpha);

		//double heuristicFactor = 0;
		//trailFactor = trail;
		//cout<<trailFactor<<"  "<<heuristicFactor<<" T H ";
		probability[i] = (trailFactor + heuristicFactor);
		//cout<<probability[i]<<"("<<functions[i]<<"), "<<endl;
	}
	//cout<<endl;
	...
	int retIndex = 0;
	auto retCRT = this->selectNeighbourProbability(probability, newFunctions, aIndex, &retIndex);
	//auto retCRT = this->selectNeighbourProbability(probability, functions, aIndex, &retIndex);
	*newSched = newScheduleResults[retIndex];
	return retCRT;
}

CourseRoomTime ImproveTable::selectNeighbourProbability(vector<double> probability, const vector<CourseRoomTime>& functions, const int& antIndex, int* returnIndex)
{
	//cout<<"Here"<<endl;
	//cout<<rt.size()<<"\t";
    ...
    mt19937::result_type seed =  std::chrono::system_clock::now().time_since_epoch().count() + antIndex;

    auto dice_randa = std::bind(std::uniform_real_distribution<double>(0,1), mt19937(seed));
...
vector<CourseRoomTime> ImproveTable::antColonySoft(const int& numberAnts, const int& numberCycles, const int& num_ber, bool accept)
{
	...
	for (int i = 0; i < numberCycles; i++)
	{
		...
		for (int j = 0; j < numberAnts; j++)
		{
			...
			//std::cout <<endl<< "Complete Ant walk in  " <<  wall1 - wall0 <<" wall seconds and "<<  cpu1 - cpu0 <<" CPU seconds"<<endl;
			cout<<"Cycle: " << i << " Ant: " << j << " Num SCV violations: " << num_scv<<endl;
		}
		if ( gBest == 0)
		{
		...
		cout<<i<<": Cycle Best is: " <<cycleBest << " Global Best is:  "<<gBest<<endl;
		int roomStabilityViolations = 0;
		int courseWorkingDayViolations = 0;
		int consecutiveLectureViolations = 0;
		int sizeViolations = 0;
		...
		auto temp = cb;
		int s = this->NumberSCV(temp, &roomStabilityViolations, &courseWorkingDayViolations, &consecutiveLectureViolations, &sizeViolations);
		cout<<"\t\t\tLocal Search Total SCV: "<<s<<" Room Stability: " << roomStabilityViolations << " Course Working Day Violations: " << courseWorkingDayViolations << " Consecutive Lecture Violations: " << consecutiveLectureViolations<< " Room Size Violations: " << sizeViolations<<endl;
...
void ImproveTable::updateAntTrail(const int& numSCV_cycleBest,  const vector<Path>& path)
{

/*	//Positive and negative rewards
	double reward = (1 / (1 + (double)numSCV_cycleBest - (double)this->globalBest )) - 1;
	reward = reward/10;

	if (this->globalBest > numSCV_cycleBest)
		reward = 1 - (1 / (1 + (double)this->globalBest  - (double)numSCV_cycleBest));*/
	...
	for (auto &p:path)
	{
		if (p.isUpdate() == true)
		{
			auto colCRT = p.getColCrt();
			auto rowCRT = p.getRowCrt();
			auto findColCRT = std::find(fullSchedule.begin(), fullSchedule.end(), colCRT);
			auto findRowCRT = std::find(fullSchedule.begin(), fullSchedule.end(), rowCRT);


			if (findRowCRT != fullSchedule.end() && findColCRT != fullSchedule.end())
			{
				int col = std::distance(fullSchedule.begin(), findColCRT);
				int row = std::distance(fullSchedule.begin(), findRowCRT);

				//cout<<rowPair<<" "<<colPair<<"-----"<<this->fullSchedule.size()<<"  "<<this->maxPeriod<<endl;
...
/*		auto rowCRT = this->fullSchedule[crtIndex];
		...
		auto rowPeriodConstraints = rowCourse.getUnavailableConstraint();
		auto colRoom = colCRT.getVenueTime().getRoom();
		auto colPeriod = colCRT.getVenueTime().getPeriod();
		std::vector<string>::iterator findRoom = std::find(rowRoomConstraints.begin(), rowRoomConstraints.end(), colRoom);
		std::vector<int>::iterator findPeriod = std::find(rowPeriodConstraints.begin(), rowPeriodConstraints.end(), colPeriod);
		if (findPeriod == rowPeriodConstraints.end() && findRoom != rowRoomConstraints.end()) //room size fits and not in unavailable
...
vector<CourseRoomTime> ImproveTable::antColonySoftThread(const int& numberAnts, const int& numberCycles, const int& num_ber, bool accept)
{
	auto schedule = this->fullSchedule;
	vector<CourseRoomTime> global = schedule;
	//cout<<"Global best is: "<<this->globalBest <<endl;
	...
	for (int i = 0; i < numberCycles; i++)
	{
		...
		//double wall0 =  SM::get_wall_time();
		//double cpu0  = SM::get_cpu_time();
		vector<Path>* cyclePath = new vector<Path>[numberAnts];
	    ...
	    for(int j = 0; j < numberAnts; j++) //create threads of number of ants to run in parallel
	    {
	    	...
	    	th.push_back(std::thread(&ImproveTable::softWalk,  this, schedule, ((i+1) *(j+1)), &antWalk[j], &n_scv[j], &cyclePath[j], accept));
		...
		//double wall1 = SM::get_wall_time();
		//double cpu1 = SM::get_cpu_time();
		//std::cout <<endl<< "ant walked in " <<  wall1 - wall0 <<" wall seconds and "<<  cpu1 - cpu0 <<" CPU seconds"<<endl;

	    //find cycle best;
	    int cycleb = n_scv[0];
	    int cycleBestIndex = 0;
	    vector<Path> pathBest = cyclePath[0];
	    for (int k = 0; k < numberAnts; k++)
	    {
	    	...
	    	cout<<"Cycle: " << i << " Ant: " << k << " Num SCV violations: " << n_scv[k]<<endl;
			if (n_scv[k] == 0)
			{
				this->globalBest = n_scv[k];
				global = antWalk[k];
				cout<<"Found Global Best is: " << this->globalBest<< " in cycle "<< i <<  " Size: "<<global.size()<<endl;
				delete[] antWalk;
				delete[] cyclePath;
				this->fullSchedule = global;
				return global;
		...
		cout<<"\t\t\t"<<i<<":  Cycle Best is: " <<cycleb << " Global Best is:  "<<this->globalBest<<  " Size: "<<global.size()<<endl;
...
vector<CourseRoomTime> ImproveTable::removeEmptyCourses(const vector<CourseRoomTime>& timetable)
{
...
vector<CourseRoomTime> ImproveTable::runImprovement(int numIterations, int numAnts, int numCycles)
{
	...
	while(i < numIterations && consecutive < 4)
	{
		...
		auto e = this->antColonySoftThread(numAnts, numCycles, numIterations, false);
...
int getRoomStabilityViolations(vector<CourseRoomTime> timetable)
{
...
CourseRoomTime ImproveTable::moveToBest2(int crtIndex, vector<CourseRoomTime>* timetable )
{
	...
	if (findCRT != tempSchedule.end() && crT.getCourse().isEmpty() == false)
	{
		...
		for (std::size_t i =0; i < this->venueTime.size(); i++)
		{
			...
			if (eventP.canAssignCourse(crt.getCourse()))
			{
				...
				SM::periodViolation(newTemp, this->maxPeriod, crt.getVenueTime().getPeriod(), &v2);
				//cout<<eventP.getPeriod()<<" Period "<<violations<<endl;
				//cout<<SM::NumberHCV(newTemp, this->maxPeriod)<<" Number HCV"<<endl;
				//cout<<" Violations: "<<eventP.getPeriod()<<" "<<violations<<" "<<crt.getVenueTime().getPeriod()<<" "<<v2<<" "<<crt.toString()<<endl;
				if (violations == 0 && v2 == 0)
				{
					//int x = -1;
					//SM::periodViolation(newTemp, this->maxPeriod, eventP.getPeriod(), &x);
					//cout<<" Period Violations "<<x<<endl;
					//cout<<endl<<eventP.getPeriod()<<" "<<violations<<" "<<SM::ThreadNumberHCV(newTemp, this->maxPeriod)<<" Number HCV "<<crt.toString()<<" "<<newTemp[indexInTS].toString()<<endl;
					int nSCurrent = this->NumberSCV(newTemp);
					if (nSCurrent < min)
					{
						min = nSCurrent;
...
vector<CourseRoomTime> ImproveTable::singleSwap(int crtOneIndex, int crtTwoIndex, vector<CourseRoomTime> schedule)
{
...
vector<CourseRoomTime> ImproveTable::singleMove(int crtOneIndex, int newPeriod, vector<CourseRoomTime> schedule)
{
	vector<CourseRoomTime> timetable = schedule;
	CourseRoomTime crtOne = timetable[crtOneIndex];
	vector<CourseRoomTime> swapList;
...
vector<CourseRoomTime> ImproveTable::bestNeighbour(int crtOneIndex,  vector<CourseRoomTime> schedule)
{
	auto best = schedule;
