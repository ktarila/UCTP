  ...
  if (myfile.is_open()) {
    while (getline(myfile, line)) {
      ...
      if (&arrayTokens[0] != NULL) {
        string *lineArray =
            &arrayTokens[0]; // pointer to vector so can access as array
        ...
        if (*lineArray == "UNAVAILABILITY_CONSTRAINTS:") {
          ...
          if (curriculum.size() == this->num_curricula )
                  ...
                  cout<<"\t"<<curriculum.size() << " " << this->num_curricula <<
          endl;
          */
          location = *lineArray;
        ...
        if (*lineArray == "ROOM_CONSTRAINTS:") {
          ...
          */
          ...
          cout << count << " " << this->num_unavailabilityConstraint;
          count = 0;
        }
        if (*lineArray == "END.") {
          location = *lineArray;
        ...
        if (location == "header") {
          ...
          // if ((&arrayTokens[1] != NULL || &arrayTokens[2] != NULL)  &&
          // &arrayTokens[3] == NULL )
          if (&arrayTokens[1] != NULL) {
            ...
            if (*lineArray == "Periods_per_day:") {
              ...
              // cout<<"num periods per day is
              // "<<this->num_periods_per_day<<endl;
            }
          ...
          if (*lineArray == "CURRICULA:") {
            ...
            for (int i = 2; i <= numItems + 1; i++) {
              ...
              std::vector<Course>::iterator findCourse =
                  std::find(courses.begin(), courses.end(), temp);
              if (findCourse !=
                  courses
                      .end()) // If course code exists add course to curriculum
              {
                cur.push_back(*tok);
                Course c = *findCourse;
                c.addCurriculaCodes(*token0); // update curriculum codes;
                // courses.remove(*findCourse);  //delete previous course from
                // list
            ...
            // cout<<"Courses added not equal to number courses in currculum
            // !!!!"<<endl;
        ...
        if (location == "UNAVAILABILITY_CONSTRAINTS:") {
          if (*lineArray == "UNAVAILABILITY_CONSTRAINTS:") {
            ...
            std::vector<Course>::iterator findCourse =
                std::find(courses.begin(), courses.end(), temp);
            if (findCourse !=
                courses.end()) // If course code exists add course to curriculum
            {
              Course c = *findCourse; // get course
              c.addUnavailableConstraint(
                  period); // update unavailable constraints
        ...
        if (location == "ROOM_CONSTRAINTS:") {
          if (*lineArray == "ROOM_CONSTRAINTS:") {
            ...
            std::vector<Course>::iterator findCourse =
                std::find(courses.begin(), courses.end(), temp);
            std::vector<Venue>::iterator findRoom =
                std::find(rooms.begin(), rooms.end(), temp1);
            if (findCourse != courses.end() &&
                findRoom != rooms.end()) // If course and room exists
            {
              Venue room = *findRoom;
              Course c = *findCourse;
...
vector<Course> Data::getCourses() const { return courses; }

vector<Curricula> Data::getCurriculum() const { return curriculum; }

int Data::getMaxDaily() const { return maxDaily; }

int Data::getMinDaily() const { return minDaily; }

string Data::getName() const { return name; }

int Data::getNumCourses() const { return num_courses; }

int Data::getNumCurricula() const { return num_curricula; }

int Data::getNumDays() const { return num_days; }

int Data::getNumPeriodsPerDay() const { return num_periods_per_day; }

int Data::getNumRoomConstraints() const { return num_roomConstraints; }

int Data::getNumRooms() const { return num_rooms; }

int Data::getNumUnavailabilityConstraint() const {
  return num_unavailabilityConstraint;
}

vector<Venue> Data::getRooms() const { return rooms; }
...
void Data::addRoomSizeConstraints() {
  ...
  while (courseToken != courses.end()) {
    ...
    if (c.getRoomConstraint().size() ==
        0) // add room size constraint to only those courses without room
           // constraints already
    {
      ...
      while (venueToken != rooms.end()) {
        ...
        if (roomCapacity >= courseSize)
          c.addRoomConstraint(
              v.getName()); // add those rooms that can fit student
        venueToken++;
      }
