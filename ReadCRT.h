...
vector<CourseRoomTime> readCRT(vector<Course> courses, vector<Venue> rooms,
                               int periods_per_day, const char *filename) {
  ...
  if (myfile.is_open()) {
    while (getline(myfile, line)) {
      ...
      // string* lineArray = &arrayTokens[0];  //pointer to vector so can access
      // as array
